     ...
     IHashService hashService, IConfiguration configuration) : IAccountService
    {
        ...
        public async Task<Response<string>> Register(RegisterDto model)
        {
            try
            {
                ...
                var existing = await context.Users.AnyAsync(x => x.Name == model.Name);
                if (existing)
                {
                    logger.LogWarning("Alredy exists this {Username} ,in time {DateTime}", model.Name,
                        DateTimeOffset.UtcNow);
                    return new Response<string>(HttpStatusCode.BadRequest,
                        $"User already exists by this username={model.Name}");
                }
